...
void sb_add_char(string_builder* sb, char c)
{
    sb->string = (char*) realloc(sb->string, sizeof(char*)*((sb->length) +  1));
	memcpy(sb->string + sb->length, &c, 1);
    sb->length ++; 
}
...
void sl_copy_if(const string_list* sl_a, string_list* sl_b, int(*comp)(const char*))
{   
    for (size_t i = 0; i < sl_a->size; i++)
    {
        if(comp(sl_a->array[i]))
        {
            sl_add(sl_b, sl_a->array[i]);
        }
    }
}
...
void sl_iterate_with_param(string_list* sl, void (*accum)(), int* sum)
{
